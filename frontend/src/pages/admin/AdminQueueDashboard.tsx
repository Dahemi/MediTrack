...
const AdminQueueDashboard: React.FC = () => {
  ...
  const fetchQueues = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/admin/queues`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('meditrack_admin_token') || ''}`,
          },
        }
      );
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/admin/queue-stats`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('meditrack_admin_token') || ''}`,
          },
