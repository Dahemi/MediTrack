...
const DoctorQueueCard: React.FC<DoctorQueueCardProps> = ({
    ...
    try {
      ...
      const [queueResponse, appointmentsResponse] = await Promise.all([
        fetch(
          ...
          {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('meditrack_doctor_token') || ''}`,
            },
          }
        ),
  ...
  const handlePauseQueue = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/doctor/queue/pause`,
        {
          method: 'POST',
          headers: {
      ...
      console.log('Pause queue response:', { status: response.status, data });
  ...
  const handleResumeQueue = async () => {
    try {
      setActionLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/doctor/queue/resume`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5">
        <div className="flex items-center justify-between">
          ...
          <div className={`flex items-center space-x-3 px-4 py-2 rounded-full ${
            queueStatus?.status === 'active' ? 'bg-green-500/20' : 'bg-red-500/20'
          } backdrop-blur-sm`}>
